...
class PublicPermohonanController extends Controller
{
    ...
    public function lookup(Request $request)
    {
        ...
        if (! $jenis->isUsaha()) {
            return response()->json([
                'error' => 'Lookup hanya diperuntukkan untuk Surat Keterangan Usaha.'
            ], 422);
        }
        ...
        if ($penduduk->tanggal_lahir) {
            $dbDob = Carbon::parse($penduduk->tanggal_lahir)->toDateString();
        }

        $inputDob = Carbon::parse($validated['tanggal_lahir'])->toDateString();
        $dobMatch = $dbDob && $dbDob === $inputDob;
    ...
    public function store(Request $request)
    {
        ...
        if ($isDomisili) {
            ...
            if ($penduduk && $penduduk->tanggal_lahir && $request->filled('tanggal_lahir')) {
                $storedDob = Carbon::parse($penduduk->tanggal_lahir)->toDateString();
                $submittedDob = Carbon::parse($request->input('tanggal_lahir'))->toDateString();

                if ($storedDob !== $submittedDob) {
                    return back()->withErrors([
                        'tanggal_lahir' => 'Tanggal lahir tidak cocok dengan NIK yang terdaftar.'
                    ])->withInput();
                }
            }
            ...
            if (! $penduduk) {
                $penduduk = Penduduk::create([
                    ...
                    'status_validasi_alamat' => 'Perlu Verifikasi',
            ...
            $dataSurat = [
                'nama_pemohon' => $penduduk?->nama_lengkap ?? $request->input('nama_lengkap'),
