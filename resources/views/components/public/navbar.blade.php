...
<nav id="navbar"
     ...
     :class="scrolled ? 'bg-white/95 backdrop-blur-md shadow-md dark:bg-slate-900/95 dark:shadow-slate-950/40 dark:border-b dark:border-slate-800' : 'bg-transparent'">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between py-3">
            ...
            <a href="{{ url('/') }}"
               ...
               aria-label="Beranda SIP Bongki">
                ...
                <div class="leading-tight min-w-0">
                    <div class="text-sm font-bold transition-colors duration-300 text-slate-800 truncate max-w-[140px] sm:max-w-none dark:text-slate-100">
                        {{ $website?->nama_website ?? 'SIP Bongki' }}
                    </div>
                    <div class="text-xs transition-colors duration-300 text-slate-500 truncate max-w-[140px] sm:max-w-none dark:text-slate-400">
            ...
            <div class="flex items-center gap-3 sm:gap-4">
                        ...
                        <li>
                            <a href="{{ $link['href'] }}"
                               class="px-3 py-2 rounded-lg text-sm font-medium transition-colors duration-200
                                      focus:outline-none text-slate-600 hover:text-primary hover:bg-primary-light dark:text-slate-300 dark:hover:text-primary-400 dark:hover:bg-slate-800">
                                {{ $link['label'] }}
                            </a>
                        </li>
                    @endforeach
                ...
                <button type="button"
                        @click="toggleTheme()"
                        class="p-2.5 text-slate-500 hover:text-amber-500 hover:bg-slate-100 rounded-full transition-all focus:outline-none cursor-pointer dark:text-slate-400 dark:hover:text-amber-400 dark:hover:bg-slate-800"
                        :title="darkMode ? 'Mode Terang' : 'Mode Gelap'"
                        aria-label="Toggle dark mode">
                    <i class="fa-solid fa-moon text-base" x-show="!darkMode"></i>
                    <i class="fa-solid fa-sun text-base text-amber-400" x-show="darkMode" style="display: none;"></i>
                </button>

                {{-- ── CTA + HAMBURGER ──────────────── --}}
                <div class="flex items-center gap-3 border-l lg:border-slate-200 lg:pl-4 border-transparent pl-0 dark:lg:border-slate-800">
                    @auth
                    ...
                    <button @click="open = !open"
                            ...
                            class="lg:hidden h-11 w-11 flex items-center justify-center rounded-xl transition-colors
                                   focus:outline-none focus:ring-2 focus:ring-primary text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800">
    ...
    <div x-show="open"
         ...
         class="lg:hidden bg-white border-t border-slate-100 shadow-xl dark:bg-slate-900 dark:border-slate-800"
        ...
        <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
            ...
            @foreach($navLinks as $link)
                <a href="{{ $link['href'] }}"
                   ...
                   class="px-4 py-3 rounded-xl text-sm font-medium text-slate-700
                          hover:bg-primary-light hover:text-primary dark:text-slate-200 dark:hover:bg-slate-800 dark:hover:text-primary-400
            ...
            <div class="pt-3 mt-2 border-t border-slate-100 px-2 pb-2 dark:border-slate-800">
                @auth
                    <a href="{{ route('dashboard') }}"
