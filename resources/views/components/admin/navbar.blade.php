...
<header class="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 lg:px-8 shrink-0">
    ...
    <div class="flex items-center gap-4">
        <button class="text-slate-500 hover:text-primary-600 lg:hidden focus:outline-none transition-colors p-2 rounded-lg hover:bg-slate-50">
    ...
    <div class="flex items-center gap-5">
        ...
        <div class="relative group">
            <button class="relative p-2 text-slate-500 hover:text-primary-600 hover:bg-slate-50 rounded-full transition-all focus:outline-none">
            ...
            {{-- Dropdown Notif --}}
            <div class="absolute right-0 mt-2 w-72 bg-white border border-slate-200 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden origin-top-right">
                <div class="px-4 py-3 bg-slate-50/80 border-b border-slate-100 font-bold text-sm text-slate-800">Pemberitahuan</div>
                <div class="py-2">
                    <a href="{{ route('admin.pengaduan.index') }}" class="flex items-center justify-between px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-primary-600 transition-colors">
                    ...
                    <a href="{{ route('admin.permohonan-surat.index') }}" class="flex items-center justify-between px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-primary-600 transition-colors">
        ...
        {{-- User Menu --}}
        <div class="relative group">
            <button class="flex items-center gap-2.5 focus:outline-none p-1 rounded-full hover:bg-slate-50 transition-colors pr-3 border border-transparent hover:border-slate-200">
            ...
            {{-- Dropdown Profile --}}
            <div class="absolute right-0 mt-2 w-56 bg-white border border-slate-200 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden origin-top-right">
                ...
                <div class="py-1.5">
                    <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-primary-600 transition-colors">
                ...
                <div class="py-1.5">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors text-left">
