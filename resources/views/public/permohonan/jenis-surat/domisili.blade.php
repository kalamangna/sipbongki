@unless($isUsaha)
    ...
    <div class="form-step hidden" data-step="3">
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-8 overflow-hidden">
            <div class="px-5 sm:px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
                <h3 class="font-bold text-slate-800 dark:text-slate-100">Langkah 3: {{ optional($jenisSurats->firstWhere('id', old('jenis_surat_id', $selected)))->nama ?? 'Detail Permohonan' }}</h3>
            </div>
            <div class="p-5 sm:p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Jenis Surat <span class="text-red-500">*</span></label>
                        <div class="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 py-2.5 text-slate-700 dark:text-slate-300 text-sm">{{ optional($jenisSurats->firstWhere('id', old('jenis_surat_id', $selected)))->nama ?? 'Surat Keterangan Domisili' }}</div>
                        @error('jenis_surat_id')<div class="mt-1 text-sm text-red-500">{{ $message }}</div>@enderror
                    </div>
                    
                    @if($isActuallyDomisili)
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Status Tempat Tinggal <span class="text-red-500">*</span></label>
                        <select name="status_tempat_tinggal" required class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm @error('status_tempat_tinggal') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Lama Tinggal <span class="text-red-500">*</span></label>
                        <input type="text" name="lama_tinggal" required class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm @error('lama_tinggal') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror" value="{{ old('lama_tinggal') }}" placeholder="Contoh: 2 Tahun">
                        @error('lama_tinggal')<div class="mt-1 text-sm text-red-500">{{ $message }}</div>@enderror
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Alamat Domisili <span class="text-red-500">*</span></label>
                        <textarea name="alamat_domisili" required rows="3" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm @error('alamat_domisili') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror" placeholder="Masukkan Alamat">{{ old('alamat_domisili') }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-3 sm:gap-6 md:col-span-1">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">RT Domisili <span class="text-red-500">*</span></label>
                            <input type="text" name="rt_domisili" required class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm @error('rt_domisili') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror" value="{{ old('rt_domisili') }}" placeholder="Contoh: 001">
                            @error('rt_domisili')<div class="mt-1 text-sm text-red-500">{{ $message }}</div>@enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">RW Domisili <span class="text-red-500">*</span></label>
                            <input type="text" name="rw_domisili" required class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm @error('rw_domisili') border-red-300 focus:border-red-500 focus:ring-red-500/20 @enderror" value="{{ old('rw_domisili') }}" placeholder="Contoh: 001">
    ...
    <div class="form-step hidden" data-step="4">
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-8 overflow-hidden">
            ...
            <div class="p-5 sm:p-6">
                <p class="mb-6 text-sm sm:text-base text-slate-600 dark:text-slate-300">Unggah dokumen persyaratan berikut untuk {{ optional($jenisSurats->firstWhere('id', old('jenis_surat_id', $selected)))->nama ?? 'permohonan Anda' }}:</p>
                <div class="space-y-6">
                    <div>
