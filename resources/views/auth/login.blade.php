...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 sm:p-10">
    ...
    <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">SIP Bongki</h1>
        <p class="text-sm text-slate-500 mt-1">Sistem Informasi & Pelayanan Kelurahan</p>
    </div>

    {{-- Alerts --}}
    @if(session('status'))
        <div class="mb-6 p-3 rounded-xl bg-emerald-50 text-emerald-700 text-sm flex items-start gap-2">
    ...
    @if($errors->any())
        <div class="mb-6 p-3 rounded-xl bg-rose-50 text-rose-700 text-sm flex items-start gap-2">
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <input id="username"
                   ...
                   required
                   autofocus
                   class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:bg-white focus:border-primary-600 focus:ring-1 focus:ring-primary-600 outline-none transition-colors">
            @error('username')
        ...
        <div>
            ...
            <div class="relative">
                <input id="password"
                       ...
                       name="password"
                       required
                       class="w-full pl-4 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:bg-white focus:border-primary-600 focus:ring-1 focus:ring-primary-600 outline-none transition-colors">
                <button type="button"
