...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">Tambah Lingkungan</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Menambahkan data lingkungan baru.</p>
        </div>
        <a href="{{ route('admin.lingkungan.index') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 shadow-sm transition-all hover:-translate-y-0.5 focus:outline-none active:scale-95 cursor-pointer dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-slate-100">
    ...
    <form action="{{ route('admin.lingkungan.store') }}" method="POST">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800">
            <div class="p-4 sm:p-6 md:p-8">
                @if ($errors->any())
                    <div class="mb-6 p-4 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-100 dark:border-rose-800/60 flex gap-3 items-start">
                        <i class="fa-solid fa-circle-exclamation text-rose-500 mt-0.5"></i>
                        <div>
                            <h4 class="text-sm font-bold text-rose-800 dark:text-rose-300">Mohon periksa kembali input Anda:</h4>
                            <ul class="text-sm text-rose-600 dark:text-rose-400 mt-1 list-disc list-inside">
