...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight dark:text-slate-100">Edit Aparatur</h2>
            <p class="text-sm text-slate-500 mt-1 dark:text-slate-400">Perbarui data perangkat atas nama <span class="font-semibold text-slate-700 dark:text-slate-200">{{ $perangkat->nama_lengkap }}</span>.</p>
        </div>
    ...
    <form action="{{ route('admin.perangkat.update', $perangkat->id) }}" method="POST" enctype="multipart/form-data" id="formPerangkat">
        @csrf
        @method("PUT")
