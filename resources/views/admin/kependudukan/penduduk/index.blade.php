...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <a href="{{ route('admin.penduduk.create') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-primary-600 text-white hover:bg-primary-700 shadow-sm transition-all hover:-translate-y-0.5 focus:outline-none cursor-pointer">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
        ...
        <a href="{{ route('admin.penduduk.index', request()->except(['aktif', 'page'])) }}"
           class="bg-white rounded-2xl p-5 ring-1 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)] hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer {{ ($aktif === null || $aktif === '') ? 'ring-2 ring-primary-500 bg-primary-50/10' : 'ring-slate-200/60' }}">
        ...
        <a href="{{ route('admin.penduduk.index', array_merge(request()->except(['page']), ['aktif' => '1'])) }}"
           class="bg-white rounded-2xl p-5 ring-1 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)] hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer {{ $aktif === '1' ? 'ring-2 ring-emerald-500 bg-emerald-50/10' : 'ring-slate-200/60' }}">
            <div class="flex items-center gap-4">
