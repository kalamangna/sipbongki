    ...
    <div class="p-6 md:p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="no_kk" placeholder="Contoh: 7371110000000000" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm" value="{{ old('no_kk', $kartuKeluarga->no_kk ?? '') }}" required>
            ...
            <div>
                ...
                <select id="kepala_keluarga_id" name="kepala_keluarga_id" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm" required>
            ...
            <div>
                ...
                <input type="text" id="rt" name="rt" placeholder="00" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm" value="{{ old('rt', $kartuKeluarga->rt ?? '') }}">
            ...
            <div>
                ...
                <input type="text" id="rw" name="rw" placeholder="00" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm" value="{{ old('rw', $kartuKeluarga->rw ?? '') }}">
            ...
            <div class="md:col-span-2">
                ...
                <select id="lingkungan_id" name="lingkungan_id" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm">
...
<div class="bg-white rounded-3xl ring-1 ring-slate-200/60 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)] overflow-hidden mb-6">
    ...
    <div class="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-slate-50/50">
        ...
        <button type="button" id="btnOpenAnggotaModal" class="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-semibold rounded-lg bg-sky-50 text-sky-700 hover:bg-sky-100 transition-colors shadow-sm">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-2xl overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50 text-slate-800">
                <h5 class="modal-title font-bold text-lg mb-0">Tambah Anggota Keluarga</h5>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <select id="modalPenduduk" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm">
                        ...
                        <div class="mt-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" id="force_reassign" class="w-4 h-4 rounded border-slate-300 text-primary-600 focus:ring-primary-500">
                    ...
                    <div>
                        ...
                        <select id="modalHubungan" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm">
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                <button type="button" class="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 shadow-sm transition-all" data-bs-dismiss="modal">
                    Batal
                </button>
                <button type="button" id="btnTambahAnggotaModal" class="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-primary-600 text-white hover:bg-primary-700 shadow-sm transition-all hover:-translate-y-0.5">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let anggota = @json(old('anggota', $kartuKeluarga->anggota ?? []));

    /*
    |--------------------------------------------------------------------------
    | Data dari Server (jika edit)
    |--------------------------------------------------------------------------
    */
    if (anggota.length === 0) {
        @if(isset($kartuKeluarga) && $kartuKeluarga->anggota->count() > 0)
            anggota = [
                @foreach($kartuKeluarga->anggota as $ag)
                {
                    id: '{{ $ag->id }}',
                    nik: '{{ $ag->nik }}',
        ...
        anggota.forEach(function (item, index) {
            ...
            tr.innerHTML = `
                ...
                <td class="px-6 py-4 text-center">
                    <button type="button" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 hover:text-rose-700 transition-colors shadow-sm" onclick="removeAnggota(${index})" ${item.hubungan === 'Kepala Keluarga' ? 'disabled title="Kepala keluarga tidak bisa dihapus dari sini"' : ''}>
            ...
            hiddenDiv.innerHTML += `
                <input type="hidden" name="anggota[${index}][id]" value="${item.id}">
