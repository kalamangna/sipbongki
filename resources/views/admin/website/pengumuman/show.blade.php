...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight dark:text-slate-100">Detail Pengumuman</h2>
            <p class="text-sm text-slate-500 mt-1 dark:text-slate-400">Informasi lengkap pengumuman Kelurahan Bongki.</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.website.pengumuman.index') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 shadow-sm transition-all hover:-translate-y-0.5 focus:outline-none cursor-pointer dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white">
                <i class="fa-solid fa-arrow-left-long text-slate-400"></i> Kembali
            </a>
            <a href="{{ route('admin.website.pengumuman.edit', $pengumuman) }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-amber-500 text-white hover:bg-amber-600 shadow-sm transition-all hover:-translate-y-0.5 focus:outline-none cursor-pointer">
                <i class="fa-solid fa-pen-to-square"></i> Edit
            </a>
            <form action="{{ route('admin.website.pengumuman.destroy', $pengumuman) }}" method="POST" class="inline m-0" onsubmit="return confirm('Yakin ingin menghapus pengumuman ini?');">
                @csrf
                @method('DELETE')
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-full dark:bg-slate-900 dark:border-slate-800">
                <div class="p-8">
                    @if($pengumuman->gambar)
                        <img src="{{ asset('storage/'.$pengumuman->gambar) }}" class="w-full h-auto max-h-[400px] object-cover rounded-xl shadow-sm mb-8 border border-slate-200 dark:border-slate-700" alt="{{ $pengumuman->judul }}">
                    @endif

                    <h1 class="text-2xl font-extrabold text-slate-900 dark:text-slate-100 mb-4 leading-tight">{{ $pengumuman->judul }}</h1>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-6 dark:bg-slate-900 dark:border-slate-800">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="font-medium text-slate-900 dark:text-slate-100">{{ $pengumuman->tanggal_publish ? $pengumuman->tanggal_publish->format('d F Y') : '-' }}</div>
                    </div>
                    <div>
                        ...
                        <div class="font-medium text-slate-900 dark:text-slate-100">{{ $pengumuman->created_at->format('d F Y') }}</div>
                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ $pengumuman->created_at->format('H:i') }} WITA</div>
                    </div>
                    <div>
                        ...
                        <div class="font-medium text-slate-900 dark:text-slate-100">{{ $pengumuman->updated_at->format('d F Y') }}</div>
                        <div class="text-xs text-slate-500 dark:text-slate-400">{{ $pengumuman->updated_at->format('H:i') }} WITA</div>
                    </div>
                </div>
            </div>
        </div>
