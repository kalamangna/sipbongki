...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Detail Pengumuman</h2>
            <p class="text-sm text-slate-500 mt-1">Informasi lengkap pengumuman Kelurahan Bongki.</p>
        </div>
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('admin.website.pengumuman.edit', $pengumuman) }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-amber-500 text-white hover:bg-amber-600 shadow-sm transition-all hover:-translate-y-0.5 focus:outline-none">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-full">
                <div class="p-8">
                    @if($pengumuman->gambar)
                        <img src="{{ asset('storage/'.$pengumuman->gambar) }}" class="w-full h-auto max-h-[400px] object-cover rounded-xl shadow-sm mb-8 border border-slate-200" alt="{{ $pengumuman->judul }}">
                    @endif

                    <h1 class="text-2xl font-extrabold text-slate-900 mb-4 leading-tight">{{ $pengumuman->judul }}</h1>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-6">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="font-medium text-slate-900">{{ $pengumuman->tanggal_publish ? $pengumuman->tanggal_publish->format('d F Y') : '-' }}</div>
                    </div>
                    <div>
                        <small class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Tanggal Dibuat</small>
                        <div class="font-medium text-slate-900">{{ $pengumuman->created_at->format('d F Y') }}</div>
                        <div class="text-xs text-slate-500">{{ $pengumuman->created_at->format('H:i') }} WITA</div>
                    </div>
                    <div>
                        <small class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Terakhir Diubah</small>
                        <div class="font-medium text-slate-900">{{ $pengumuman->updated_at->format('d F Y') }}</div>
                        <div class="text-xs text-slate-500">{{ $pengumuman->updated_at->format('H:i') }} WITA</div>
                    </div>
                </div>
            </div>
        </div>
