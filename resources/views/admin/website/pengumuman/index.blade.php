...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Data Pengumuman</h2>
            <p class="text-sm text-slate-500 mt-1">Kelola pengumuman publik SIP Bongki.</p>
        </div>
        <a href="{{ route('admin.website.pengumuman.create') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-primary-600 text-white hover:bg-primary-700 shadow-sm transition-all hover:-translate-y-0.5 focus:outline-none">
            ...
            <table class="w-full text-sm text-left text-slate-600">
                <thead class="text-xs font-semibold text-slate-500 uppercase bg-slate-50/80">
                    <tr>
                        ...
                        <th class="px-6 py-4 border-b border-slate-100">Judul Pengumuman</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="px-6 py-4 text-center font-medium">{{ $pengumumen->firstItem() + $index }}</td>
                        <td class="px-6 py-4 text-center">
                            @if($pengumuman->gambar)
                                <img src="{{ asset('storage/'.$pengumuman->gambar) }}" class="w-20 h-14 object-cover rounded-lg shadow-sm mx-auto" alt="Gambar">
                            @else
                        ...
                        <td class="px-6 py-4">
                            <div class="font-bold text-slate-900 mb-1">{{ $pengumuman->judul }}</div>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.website.pengumuman.show', $pengumuman) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-sky-50 text-sky-600 hover:bg-sky-100 hover:text-sky-700 transition-colors focus:outline-none" title="Detail">
                                    <i class="fa-solid fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.website.pengumuman.edit', $pengumuman) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 hover:text-amber-700 transition-colors focus:outline-none" title="Edit">
