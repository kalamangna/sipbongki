...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah Pengumuman</h2>
            <p class="text-sm text-slate-500 mt-1">Tambahkan pengumuman terbaru Kelurahan Bongki.</p>
        </div>
        ...
        <form action="{{ route('admin.website.pengumuman.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-4 sm:p-6 md:p-8 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="judul" value="{{ old('judul') }}" placeholder="Masukkan judul pengumuman" class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm">
                </div>

                {{-- Isi --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Isi Pengumuman</label>
                    <textarea name="isi" rows="8" placeholder="Tuliskan isi pengumuman..." class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 px-4 py-3 transition-colors shadow-sm">{{ old('isi') }}</textarea>
                </div>
            ...
            <div class="px-4 sm:px-6 md:px-8 py-4 border-t border-slate-100 bg-slate-50/50 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3">
                <a href="{{ route('admin.website.pengumuman.index') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 shadow-sm transition-all focus:outline-none active:scale-95 cursor-pointer">
                    Batal
                </a>
