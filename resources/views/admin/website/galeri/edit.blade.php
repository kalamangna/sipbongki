...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight dark:text-slate-100">Edit Galeri</h2>
            <p class="text-sm text-slate-500 mt-1 dark:text-slate-400">Perbarui dokumentasi kegiatan Kelurahan Bongki.</p>
        </div>
    ...
    <form action="{{ route('admin.website.galeri.update', $galeri->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800">
            <div class="p-4 sm:p-6 md:p-8 space-y-6">
                ...
                {{-- Judul --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5 dark:text-slate-300">Judul Dokumentasi</label>
                ...
                {{-- Deskripsi --}}
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5 dark:text-slate-300">Deskripsi (Opsional)</label>
                    <textarea name="deskripsi" rows="4" placeholder="Keterangan dokumentasi..." class="w-full bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary px-4 py-3 transition-colors shadow-sm dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100 dark:placeholder-slate-500">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- Gambar --}}
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1.5 dark:text-slate-300">Ganti Foto (Opsional)</label>
                        <div class="flex flex-col gap-4">
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400">Format: JPG, JPEG, PNG. Maksimal 2MB. Kosongkan jika tidak ingin mengubah foto.</p>
                            
                            <div class="flex flex-wrap gap-4 mt-2">
                                @if($galeri->gambar)
                                    <div>
                                        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Foto Saat Ini</p>
                                        <img src="{{ asset('storage/'.$galeri->gambar) }}" class="w-[200px] h-[130px] object-cover rounded-xl shadow-sm border border-slate-200 dark:border-slate-700" alt="Current Image">
                                    </div>
                                @endif
                                <div>
                                    <p id="preview-label" class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider hidden">Foto Baru</p>
                                    <img id="preview" src="#" class="hidden w-[200px] h-[130px] object-cover rounded-xl shadow-sm border border-emerald-200 dark:border-emerald-700">
                                </div>
                            </div>
                        </div>
                    </div>
