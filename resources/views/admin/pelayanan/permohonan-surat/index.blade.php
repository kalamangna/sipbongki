...
<div class="w-full">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 mb-6">
        ...
        <a href="{{ route('admin.permohonan-surat.index', array_merge(request()->except(['page']), ['status' => request('status') === 'Menunggu' ? null : 'Menunggu'])) }}" 
           class="bg-white rounded-2xl p-4 sm:p-5 ring-1 transition-all hover:-translate-y-0.5 hover:shadow-md block {{ request('status') === 'Menunggu' ? 'ring-amber-500 shadow-md ring-2' : 'ring-slate-200/60 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)]' }}">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2 mt-0.5">
                        <span class="text-xl sm:text-2xl font-bold text-slate-900">{{ number_format($stats['menunggu']) }}</span>
        ...
        <a href="{{ route('admin.permohonan-surat.index', array_merge(request()->except(['page']), ['status' => request('status') === 'Diproses' ? null : 'Diproses'])) }}" 
           class="bg-white rounded-2xl p-4 sm:p-5 ring-1 transition-all hover:-translate-y-0.5 hover:shadow-md block {{ request('status') === 'Diproses' ? 'ring-sky-500 shadow-md ring-2' : 'ring-slate-200/60 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)]' }}">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2 mt-0.5">
                        <span class="text-xl sm:text-2xl font-bold text-slate-900">{{ number_format($stats['diproses']) }}</span>
                        <span class="text-xs text-slate-400 font-medium">Permohonan</span>
                    </div>
                </div>
            </div>
        ...
        <a href="{{ route('admin.permohonan-surat.index', array_merge(request()->except(['page']), ['status' => request('status') === 'Selesai' ? null : 'Selesai'])) }}" 
           class="bg-white rounded-2xl p-4 sm:p-5 ring-1 transition-all hover:-translate-y-0.5 hover:shadow-md block {{ request('status') === 'Selesai' ? 'ring-emerald-500 shadow-md ring-2' : 'ring-slate-200/60 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)]' }}">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2 mt-0.5">
                        <span class="text-xl sm:text-2xl font-bold text-slate-900">{{ number_format($stats['selesai']) }}</span>
                        <span class="text-xs text-slate-400 font-medium">Permohonan</span>
                    </div>
                </div>
            </div>
        ...
        <a href="{{ route('admin.permohonan-surat.index', array_merge(request()->except(['page']), ['status' => request('status') === 'Ditolak' ? null : 'Ditolak'])) }}" 
           class="bg-white rounded-2xl p-4 sm:p-5 ring-1 transition-all hover:-translate-y-0.5 hover:shadow-md block {{ request('status') === 'Ditolak' ? 'ring-rose-500 shadow-md ring-2' : 'ring-slate-200/60 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)]' }}">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2 mt-0.5">
                        <span class="text-xl sm:text-2xl font-bold text-slate-900">{{ number_format($stats['ditolak']) }}</span>
                        <span class="text-xs text-slate-400 font-medium">Permohonan</span>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-slate-600 min-w-[850px]">
                <thead class="text-xs font-semibold text-slate-500 uppercase bg-slate-50/80">
                    <tr>
                        ...
                        <th class="px-4 sm:px-6 py-4 border-b border-slate-100 whitespace-nowrap">No. Permohonan</th>
                        <th class="px-4 sm:px-6 py-4 border-b border-slate-100 whitespace-nowrap">Tanggal</th>
                        <th class="px-4 sm:px-6 py-4 border-b border-slate-100 whitespace-nowrap">Pemohon</th>
                        <th class="px-4 sm:px-6 py-4 border-b border-slate-100 whitespace-nowrap">Jenis Surat</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-4 sm:px-6 py-3.5 sm:py-4 font-mono font-medium text-slate-900 whitespace-nowrap">{{ $permohonan->nomor_permohonan }}</td>
