...
<div class="w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 tracking-tight dark:text-slate-100">Edit Persuratan</h2>
            <p class="text-sm text-slate-500 mt-1 dark:text-slate-400">Perbarui data permohonan surat <span class="font-semibold text-slate-700 dark:text-slate-200">#{{ $permohonanSurat->nomor_permohonan }}</span></p>
        </div>
        <a href="{{ route('admin.permohonan-surat.index') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 shadow-sm transition-all hover:-translate-y-0.5 focus:outline-none active:scale-95 cursor-pointer dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700">
