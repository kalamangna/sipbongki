...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        <h3 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-1">Laporan Persuratan</h3>
        <p class="text-sm text-slate-500 dark:text-slate-400">Rekapitulasi seluruh pelayanan persuratan Kelurahan Bongki</p>
    </div>
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-5 mb-6">
    <div class="bg-white rounded-2xl p-5 ring-1 ring-slate-200/60 shadow-sm text-center dark:bg-slate-900 dark:ring-slate-800">
        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Total Permohonan</p>
        <p class="text-3xl font-extrabold text-slate-900 dark:text-slate-100">{{ number_format($statistik['total']) }}</p>
    </div>
    <div class="bg-white rounded-2xl p-5 ring-1 ring-slate-200/60 shadow-sm text-center dark:bg-slate-900 dark:ring-slate-800">
        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Menunggu</p>
        <p class="text-3xl font-extrabold text-amber-500 dark:text-amber-400">{{ number_format($statistik['menunggu']) }}</p>
    </div>
    <div class="bg-white rounded-2xl p-5 ring-1 ring-slate-200/60 shadow-sm text-center dark:bg-slate-900 dark:ring-slate-800">
        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Diproses</p>
        <p class="text-3xl font-extrabold text-sky-600 dark:text-sky-400">{{ number_format($statistik['diproses']) }}</p>
    </div>
    <div class="bg-white rounded-2xl p-5 ring-1 ring-slate-200/60 shadow-sm text-center dark:bg-slate-900 dark:ring-slate-800">
        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Selesai</p>
        <p class="text-3xl font-extrabold text-emerald-600 dark:text-emerald-400">{{ number_format($statistik['selesai']) }}</p>
    </div>
</div>

{{-- FILTER BAR (1 BARIS) --}}
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-6 dark:bg-slate-900 dark:border-slate-800">
    <form method="GET" action="{{ route('admin.laporan.persuratan') }}" class="flex flex-col xl:flex-row items-stretch xl:items-center gap-3">
        ...
        {{-- Rentang Tanggal --}}
        <div class="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 shrink-0 dark:bg-slate-800 dark:border-slate-700">
            <span class="text-xs font-semibold text-slate-400 mr-1"><i class="fa-regular fa-calendar text-[11px]"></i></span>
            <input type="date" name="tanggal_awal" value="{{ request('tanggal_awal') }}"
                class="bg-transparent text-xs text-slate-800 dark:text-slate-200 focus:outline-none border-0 p-0" title="Tanggal Mulai">
            <span class="text-xs text-slate-400 font-medium px-1">s/d</span>
            <input type="date" name="tanggal_akhir" value="{{ request('tanggal_akhir') }}"
                class="bg-transparent text-xs text-slate-800 dark:text-slate-200 focus:outline-none border-0 p-0" title="Tanggal Selesai">
        </div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-6 dark:bg-slate-900 dark:border-slate-800">
    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-800/50">
        <h5 class="text-sm font-bold text-slate-800 dark:text-slate-100">Rekap Berdasarkan Jenis Surat</h5>
    </div>
    <div class="p-6 grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        <div class="text-center p-4 rounded-xl border border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-800/50">
            <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1 truncate" title="{{ $item->nama }}">{{ $item->nama }}</p>
            <p class="text-2xl font-extrabold text-{{ $color }}-600">{{ number_format($item->permohonan_surats_count) }}</p>
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Permohonan</p>
        </div>
        @empty
        <div class="col-span-4 text-center text-sm text-slate-400 dark:text-slate-500 py-6">Belum ada data jenis surat.</div>
        @endforelse
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800">
    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3 dark:border-slate-800 dark:bg-slate-800/50">
        <h5 class="text-sm font-bold text-slate-800 dark:text-slate-100">Data Permohonan Surat</h5>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-slate-600 dark:text-slate-300">
            <thead class="text-xs font-semibold text-slate-500 uppercase bg-slate-50/50 dark:bg-slate-800/50 dark:text-slate-400">
                <tr>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800 w-10">No</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800" style="min-width:180px">No. Permohonan</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800" style="min-width:130px">No. Surat</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800" style="min-width:160px">Pemohon</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800" style="min-width:200px">Jenis Surat</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800" style="min-width:110px">Tanggal</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800" style="min-width:180px">Penandatangan</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800 text-center">Status</th>
                    <th class="px-4 py-4 border-b border-slate-100 dark:border-slate-800 text-center w-16">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td class="px-4 py-3 font-mono text-xs text-slate-600 dark:text-slate-400">{{ $item->nomor_permohonan }}</td>
                    <td class="px-4 py-3 font-mono text-xs text-slate-500 dark:text-slate-400">{{ $item->nomor_surat ?: '-' }}</td>
                    <td class="px-4 py-3">
                        <p class="font-medium text-slate-900 dark:text-slate-100 leading-tight">{{ optional($item->penduduk)->nama_lengkap ?? data_get($item->data_surat, 'nama_lengkap') ?? '-' }}</p>
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{{ optional($item->penduduk)->nik ?? data_get($item->data_surat, 'nik') ?? '-' }}</p>
                    </td>
                    <td class="px-4 py-3 text-slate-700 dark:text-slate-300">{{ optional($item->jenisSurat)->nama }}</td>
                    <td class="px-4 py-3 text-slate-500 dark:text-slate-400">{{ optional($item->tanggal_permohonan)->format('d/m/Y') }}</td>
                    <td class="px-4 py-3">
                        <p class="font-medium text-slate-800 dark:text-slate-200 leading-tight">{{ optional($item->penandatangan)->nama_lengkap ?? '-' }}</p>
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{{ optional(optional($item->penandatangan)->jabatan)->nama ?? '' }}</p>
                    </td>
                    <td class="px-4 py-3 text-center">
                        @switch($item->status)
                            @case('Menunggu')
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-100 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300">Menunggu</span>
                                @break
                            @case('Diproses')
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-sky-100 text-sky-700 dark:bg-sky-950/60 dark:text-sky-300">Diproses</span>
                                @break
                            @case('Selesai')
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300">Selesai</span>
                                @break
                            @default
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold bg-rose-100 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300">Ditolak</span>
                        @endswitch
                    </td>
                    <td class="px-4 py-3 text-center">
                ...
                @empty
                <tr>
                    <td colspan="9" class="px-6 py-10 text-center text-sm text-slate-400 dark:text-slate-500">
