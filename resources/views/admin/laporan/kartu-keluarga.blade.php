...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
        <a href="{{ route('admin.laporan.index') }}"
            class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl bg-white text-slate-600 hover:bg-slate-50 border border-slate-200 shadow-sm transition-all active:scale-95 focus:outline-none">
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-5 mb-6">
    <div class="bg-white rounded-2xl p-4 sm:p-5 ring-1 ring-slate-200/60 shadow-sm text-center">
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Total KK</p>
        <p class="text-2xl sm:text-3xl font-extrabold text-slate-900">{{ number_format($statistik['total_kk']) }}</p>
    </div>
    <div class="bg-white rounded-2xl p-4 sm:p-5 ring-1 ring-slate-200/60 shadow-sm text-center">
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Total Anggota</p>
        <p class="text-2xl sm:text-3xl font-extrabold text-emerald-600">{{ number_format($statistik['total_anggota']) }}</p>
    </div>
    <div class="bg-white rounded-2xl p-4 sm:p-5 ring-1 ring-slate-200/60 shadow-sm text-center">
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">KK Aktif</p>
        <p class="text-2xl sm:text-3xl font-extrabold text-sky-600">{{ number_format($statistik['kk_aktif']) }}</p>
    </div>
    <div class="bg-white rounded-2xl p-4 sm:p-5 ring-1 ring-slate-200/60 shadow-sm text-center">
        <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Rata-rata / KK</p>
        <p class="text-2xl sm:text-3xl font-extrabold text-amber-600">{{ $statistik['rata_anggota'] }}</p>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 mb-6">
    <form method="GET" action="{{ route('admin.laporan.kartu-keluarga') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <div class="flex items-center gap-2 shrink-0">
            <button type="submit"
                class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-xl bg-slate-800 text-white hover:bg-slate-700 shadow-sm transition-all active:scale-95 focus:outline-none cursor-pointer">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-slate-600 min-w-[600px]">
            <thead class="text-xs font-semibold text-slate-500 uppercase bg-slate-50/50">
                <tr>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 w-12">No</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100">Lingkungan</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 text-center">Jumlah KK</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 text-center">Jumlah Penduduk</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td class="px-4 sm:px-6 py-3.5 sm:py-4 font-medium text-slate-800">{{ $item->nama }}</td>
                ...
                @empty
                <tr><td colspan="4" class="px-4 sm:px-6 py-8 text-center text-sm text-slate-400">Belum ada data.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
    <div class="p-4 sm:p-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-slate-600 min-w-[800px]">
            <thead class="text-xs font-semibold text-slate-500 uppercase bg-slate-50/50">
                <tr>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 w-12">No</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100">No. KK</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100">Kepala Keluarga</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 text-center">Anggota</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100">Lingkungan</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 text-center">RT/RW</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 text-center">Status</th>
                    <th class="px-4 sm:px-6 py-4 border-b border-slate-100 text-center">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td class="px-4 sm:px-6 py-3.5 sm:py-4 font-mono text-xs text-slate-600">{{ $kk->no_kk }}</td>
                    <td class="px-4 sm:px-6 py-3.5 sm:py-4 font-medium text-slate-900">{{ optional($kk->kepalaKeluarga)->nama_lengkap ?? '-' }}</td>
                    <td class="px-4 sm:px-6 py-3.5 sm:py-4 text-center">
                    ...
                    <td class="px-4 sm:px-6 py-3.5 sm:py-4 text-center text-slate-500">{{ $kk->rt }}/{{ $kk->rw }}</td>
                    <td class="px-4 sm:px-6 py-3.5 sm:py-4 text-center">
                        @if($kk->aktif)
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-4 sm:px-6 py-10 text-center text-sm text-slate-400">
